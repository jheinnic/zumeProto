	...
	for (nextRound of roundOrder) {
		...
		for (const nextChecked of fixedPlayers) {
			...
			if (fixedPlayers.has(nextPaired)) {
				printReject(rotationRounds, nextRound);
				mismatch = true;
				break;
			}
		}
...
var counter = 0
var interval = 50000;
function printReject(rotationRounds, nextRound) {
	if ((++counter % interval) > 0) {
		return;
	}

	let tempSet = '{';
	rotationRounds.forEach(
		(rotationRound) => { tempSet += `${roundMapsList.indexOf(rotationRound)}, `; }
	);
	tempSet += roundMapsList.indexOf(nextRound) + '}';
	console.log(`Rejected ${util.inspect(tempSet)}`);
}
