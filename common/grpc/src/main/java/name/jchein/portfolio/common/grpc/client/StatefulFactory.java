...
//
///**
//* GRpcClientFactory is responsible for inspecting all GRpc Client implementations and building out
...
//
//	private static final String DEFAULT_PACKAGE = "name.jchein";
//
...
//
//	/**
//	 * Will register GRPC Client Framework SmartLifecycle Beans using GRPC Protocol stubs found on the classpath
//	 * under provided package name.  Will not scan name.jchein.grpc.proto by default unless requested to do so,
//	 * although there is a zero-argument constructor that does precisely that.
//	 *
//	 * @param packages This list of packages to scan for bindable GRPC ProtoBuf RPC service stubs.
//	 */
//	public StatefulFactory(String... packages) {
//		this.packages = packages;
//	}
//
//	/** Resolver that injects the GRpcStub<GRPCProto> and GRpcClientSmartLifecycle<GRPCProto> for a given GRPCProto
//	 * definition, where the GRPCProto sub-type in question typically is the type of a field annotated with 
//	 * @GRpcClient, but could also have been explicitly given by annotation arguments themselves.
//	 */
//	class GRpcClientAnnotationResolver extends QualifierAnnotationAutowireCandidateResolver {
//
//		@Override
...
//
//			boolean isGrpcClient = false;
//			String controllerId = null;
//			Type genericFieldType = null;
//			String fieldName = null;
//			GRpcClient grpcClientAnnotation = null;
//
//			// If this is GRpc Protobuf RPC client reference, parse out Annotation and Type information
//			if (field != null) {
//				if (hasGRpcProtoType(field)) {
//					grpcClientAnnotation = field.getAnnotation(GRpcClient.class);
//					genericFieldType = field.getGenericType();
//					fieldName = field.getName();
//					isGrpcClient = true;
//				}
//			} else if (methodParameter != null) {
//				if (hasGRpcProtoType(methodParameter)) {
//					grpcClientAnnotation = methodParameter.getParameterAnnotation(GRpcClient.class);
...
//
//			// TODO
//			// If this is a GRpcClient field reference, then resolve bean reference
//			//
//			if (isGrpcClient) {
//				// Determine the controllerId - either explicit or derived
//				controllerId = getControllerId(grpcClientAnnotation);
...
//		}
//
//		private String getControllerId(GRpcClient grpcClientAnnotation) {
//			String controllerId = (grpcClientAnnotation != null ) ? grpcClientAnnotation.value() : null;
//			return controllerId;
//		}
//
//		/**
...
//		 * @return
//		 */
//		private boolean hasGRpcProtoType(Field field) {
...
//		 * @return
//		 */
//		private boolean hasGRpcProtoType(MethodParameter methodParameter) {
...
//				log.error("Field {} isn't parametrized", fieldName);
...
//
//
//	/* Set the GRPcClientAnnotationResolver to resolve all GRpcClient annotations.  This scopes the set of 
//	 * known GRPC Protobuf RPC stubs for which we must locate client implementations during the classpath 
//	 * scan phase in order to satisfy the application's requirements.
//	 *
//	 * (non-Javadoc)
...
//		bf.setAutowireCandidateResolver(new GRpcClientAnnotationResolver());
//	}
//
//	@Override
...
//
//	/*
//	 * Override postProcessBeanDefinitionRegistry to dynamically generate bean registrations for each GRPCClient found
//	 * that has one or more @GRpcClient autowiring annotations requiring it.  Also generate bean registrations for a
//	 * SmartLifecycle handler to open and close each required ManagedChannel appropriately.
//	 *
//	 * (non-Javadoc)
...
//			throws BeansException {
//		log.debug("postProcessBeanDefinitionRegistry : enter");
//		try {
//
//			// Reflect over search packages.  
//			//
//			Reflections reflections = new Reflections((Object[])this.packages);
//
//			// TODO: The required code will be similar but different to what's commented out below.
//			//
////			// 
...
////			//
////			Map<String, Class<?>>  = new HashMap<String, Class<?>>();
...
//		}
//		log.debug("postProcessBeanDefinitionRegistry : exit");
//	}
//
//	/**
...
//
//			if (bf.isAbstract()) {
//				log.debug("Skipping abstract bean " + bfName);
//				continue;
//			}
//
...
//
//			if (clazz == null) {
//				log.debug("Unable to resolve class for bean " + bfName);
//				continue;
//			}
//
...
//		//
//		log.debug("Mapped \"{}\" to repo \"{}\", beanId=\"{}\"", entityType.getName(), value, bfName);
//
...
//			Class<?> clazz) {
//		log.debug("Found StatefulController, class = \"{}\", beanName = \"{}\"", clazz.getName(), bfName);
//
//		// Ctrl -> Entity
//		//
...
//		//
//		if (repoBeanId == null) {
//			log.warn("Unable to find Spring Data Repository for {}, using an in-memory persister", managedClass.getName());
...
//				//
//				if (binder == null) {
//					log.error("Unable to locate binder: {}", entry.getKey());
...
//		}
//
//		log.debug("Mapping events and transitions for {}", statefulControllerClass);
//
